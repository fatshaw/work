...
my $ip;
my $progname = "stateserver";
my $prognameCtrl = "./StateServerCtrl.sh";
my $workpath = "/home/xcj/server/stateserver/trunk/bin";
...
sub get_ip{
   my $ipstr = `/sbin/ifconfig  | grep 'inet addr:'|grep Bcast | grep '10.'|awk '{print substr(\$2,6)}'`;
   chomp $ipstr;
   return $ipstr;
}
