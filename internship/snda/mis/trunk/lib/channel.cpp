...
void MISChannel::addInterfaceFrequencyLimiter(int type, time_t timeout,int times,string & consumer_key, string & interfaceName)
{
	char prefix[200];
	sprintf(prefix,"%s|%s",consumer_key.c_str(),interfaceName.c_str());
	m_freq_limiter.addLimiter(prefix,type,timeout,times);
}
