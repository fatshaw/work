...
void MD5::md5(const string& data, string& result) {
	unsigned char digest[16];
	MD5_CTX context;
	init(context);
	int wlen = 0;
	update(context, (unsigned char*)data.c_str(), (unsigned int)data.length());
	final(context, (unsigned char*)digest);
...
void MD5::final(MD5_CTX& context, UCHAR digest[16]) {
...
//////////////////////////////////////////////////////////////////////////
//private:
void MD5::transform(UINT state[4],UCHAR block[64]) {
...
void MD5::encode(UCHAR *output,UINT *input,UINT len)
{
	unsigned int i, j;
...
void MD5::decode(UINT *output,UCHAR *input,UINT len) {
	unsigned int i, j;
