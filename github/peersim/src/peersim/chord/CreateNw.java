...
public class CreateNw implements Control {
	...
	public boolean execute() {
		for (int i = 0; i < Network.size(); i++) {
			...
			cp.chordId = new BigInteger(new Integer(i).toString());
	...
	public Node findid(BigInteger nodeid)
	{
		return Network.get(nodeid.intValue());
	}
	...
	public void createFingerTable() {
		...
		for (int i = 0; i < Network.size(); i++) {
			...
			for (j = 0; j < idLength; j++) {
				...
				cp.fingerTable[j] = findid(pot);//findId(pot, 0, Network.size() - 1);
			}
		}
	}
}
