...
public:
	bool cudaGlobalIncrementalExpand(GlobalGrid & g, int size);
	void cudaGlobalIncrementalAdjust(double timeArrive);
	bool cudaGlobalIncrementalRangeQuery(GlobalGrid & g,list<string>&seedlist,int size, int type);
	void assigncudaGrid(list<string>&globalKey);
	void assigncudaGrid(cudaGrid * grid, GlobalGrid & global);
	void cudaBuildCluster();
	bool cudaExpandCluster(GlobalGrid & globalGrid, list<string>&seedList, LocalGrid & localGrid, int type = EXPAND);
	void cudaEplisonRangeQuery(GlobalGrid & globalGrid, list<string>&seedlist,int size, int type);
private:
	...
	bool localExpandCluster(GlobalGrid & globalGrid, list<string>&seedlist, LocalGrid & localGrid, int type = EXPAND, cudaGrid * cudaArray = 0);
...
private:
	int * h_result;
	cudaGrid * h_cudaArray;
	cudaGrid * d_cudaArray;
	int * d_result;
	cudaGrid * d_curGrid;
	cudaGrid * h_curGrid;
	int * h_pointCnt;
