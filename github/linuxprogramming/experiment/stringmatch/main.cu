...
int main( int argc, char** argv) 
{
	...
	//important : add 1 so that we should take the '\n' into consideration in the map function.
	//a little bit weird.
	...
	SM_KEY_T key;
	key.ptrFile = d_filebuf;//all the key with the same file address, i think it can be modified.
	key.ptrKeyword = d_keyword;//ptrKeyword stores keyword we need compare
	SM_VAL_T val;
	...
	while (1)
	{
		...
		p += blockSize;
		for (; *p != '\n' && *p != '\0' && *p != '\t' && *p != ' '; p++);	// stop only when we meet '\n' or '\0'.  
		if (*p != '\0')
		{
			++p;
