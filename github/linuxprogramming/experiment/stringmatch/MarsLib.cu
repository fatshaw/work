		   ...
		   int		valSize)
{
	...
	{
		//check whether memory is enough,if not then reallocate memory.
		if (dataChunkSize*curChunkNum.x < (curOffset.x + keySize))
			spec->inputKeys = (char*)realloc(spec->inputKeys, (++curChunkNum.x)*dataChunkSize);
		memcpy(spec->inputKeys+curOffset.x, key, keySize);//copy key to the inputkey in the spec struct.
		...
		//inputOffsetSize: each record stores four int variable.
	...
	else
	{
		...
		//chunk number updated
		curChunkNum.x++;
		curChunkNum.y++;
		curChunkNum.z++;
	}
	
	//update current pair of key and value offset.
	...
	//static variable , store offset of all the records 
	curOffset.x += keySize;
	curOffset.y += valSize;

	spec->inputRecordCount++;//record count + 1
}
...
{
	int index = TID;//current threadid:BLOCK_ID * blockDim.x + THREAD_ID
	//current task keysize and value size need be updated
	interKeysSizePerTask[index] += keySize;
	interValsSizePerTask[index] += valSize;
	interCountPerTask[index]++;//update keys count of current task.
}
				 ...
				 int*	curIndex)
{
	...
	//	psKeySizes:total size of previous keys
	//	psValSize:total size of previous values
	//	keyValOffsets[index]: current thread offset
	/*	important: 	interKeys : store the intermediate keys 
	 * 				interVals : store the intermediate values. 
	 * */
	...
	for (int i = 0; i < keySize; ++i)
		pKeySet[i] = sKey[i];//store key
	for (int i = 0; i < valSize; ++i)
		pValSet[i] = sVal[i];//store value

	//current thread offset will add keySize and valSize.
	...
	int l_curIndex = curIndex[index];//record count of current thread 
	int l_psCounts = psCounts[index];//record count of all previous threads
	int l_curPs = l_curIndex + l_psCounts;//record count of all the threads previous including the current thread.
	int4 l_interOffsetSizes1 = interOffsetSizes[l_curPs];//current offset
	int4 l_interOffsetSizes2 = interOffsetSizes[l_curPs-1];//last offset

	if (l_curIndex != 0)
	{
	     l_interOffsetSizes1.x = (l_interOffsetSizes2.x + l_interOffsetSizes2.y);//x offset of key,y keysize,x+y = offset of new key
	     l_interOffsetSizes1.z = (l_interOffsetSizes2.z + l_interOffsetSizes2.w);//z offset of value,w keysize,z+w = offset of new value
	}
	
	
	/*
	 * set inter offset for each record.
	 * 	interoffset: 	x = offset of key
	 *                   y = keysize
	 *                   z = offset of value
	 *                   w = valuesize
	 *          different from keyValOffset which only stores the offset for key and value in the current task
	 *                                 while interoffset stores the offset for all the key and value intermediate data.
	 * */
...
__global__ void MapperCount(char*	inputKeys,
			...
			int*	interCountPerTask,
			int recordNum, 
			int recordsPerTask,//default 1
			int taskNum)
{
	int index = TID;//current position of the input key and value
	int bid = BLOCK_ID;
	int tid = THREAD_ID;
	if (index*recordsPerTask >= recordNum) return;
	int recordBase = bid * recordsPerTask * blockDim.x; //the base address of the current block.
	int terminate = (bid + 1) * (recordsPerTask * blockDim.x);//the end address of the current block
	if (terminate > recordNum) terminate = recordNum;

	for (int i = recordBase + tid; i < terminate; i+=blockDim.x)//coalesced access 无缝访问
	{
		int cindex = i;
		int4 offsetSize = inputOffsetSizes[cindex];//current key offset
		char *key = inputKeys + offsetSize.x;//get current key
		char *val = inputVals + offsetSize.z;//get current value
		/*	
		 * map_count: in the map.cu denoted as MAP_COUNT_FUNC
		 * interKeysSizePerTask: store the keysize needed by each thread.
		 * interValsSizePerTask: store the valuesize needed by each thread.
		 * interCountPerTask:	store the key count in the each thread.
		 *   
		 *  after map_count we can know the size of intermediate data so that we can allocate cuda memory for it.
		 * */
		map_count(key,
		          val,
			  offsetSize.y,//keysize
			  offsetSize.w,//valuesize
			  interKeysSizePerTask,//three intermediate data.
...
__global__ void Mapper(char*	inputKeys,
		   ...
		   int	taskNum)
{
	int index = TID;//index in the total thread
	int bid = BLOCK_ID;//block id
	int tid = THREAD_ID;//current thread id in the block

	if (index*recordsPerTask >= recordNum) return;
	int recordBase = bid * recordsPerTask * blockDim.x;//base address
	int terminate = (bid + 1) * (recordsPerTask * blockDim.x);//end address
	if (terminate > recordNum) terminate = recordNum;

	int l_psCounts = psCounts[index];//get key count before current key.
	int4 l_interOffsetSizes = interOffsetSizes[l_psCounts];//get offset of current key in the intermediate data
	l_interOffsetSizes.x = psKeySizes[index];//update key offset 
	l_interOffsetSizes.z = psValSizes[index];//update value offset
	interOffsetSizes[l_psCounts] = l_interOffsetSizes;//store the offset 

	for (int i = recordBase + tid; i < terminate; i+=blockDim.x)//coalesced access. span is blockDim.x
	{
		int cindex =  i;

		int4 offsetSize = inputOffsetSizes[cindex];//offset of current key and value
		char *key = inputKeys + offsetSize.x;//get current key through offset
		char *val = inputVals + offsetSize.z;//get current value through offset
	/*	begin map: defined in the map.cu as MAP_FUNC
	 * 	
	 * */
		map(key,
		val,
...
int startMap(Spec_t* spec)
{
	...
	/* 	g_spec->inputOffsetSizes[h_inputRecordCount-1].x: size of all the keys before the last one.
	 *    g_spec->inputOffsetSizes[h_inputRecordCount-1].y: size of the last key.
	 *    so we 
	 *    g_spec->inputOffsetSizes[h_inputRecordCount-1].x +
	 *     		 g_spec->inputOffsetSizes[h_inputRecordCount-1].y =  size of the total keys.
	 *       
	 *       the same with the value size   
	 *       
	 * */
	...
	//pair of key and value <key,value>:such as <"abc",1> in the word count
	char*	h_inputKeys = g_spec->inputKeys;//input key such as "abc", "Project", "Mars"
	char*	h_inputVals = g_spec->inputVals;//input value such as 1 
	int4*	h_inputOffsetSizes = g_spec->inputOffsetSizes;//input offset size : contains offset and size of key and value
	...
	//copy inputkey and value and offset to the cuda memory
	...
	//d_interKeysSizePerTask: a array with length of h_actualNumThreads.
	// it means each thread will have a keysize value
	...
	//the same with keysize. it stores the keys count of each thread.
	...
	/*	begin to mappercount in order to calculate intermediate data size. 
	 * */
	MapperCount<<<h_dimGrid, h_dimBlock>>>(d_inputKeys,
                                               d_inputVals,
					       d_inputOffsetSizes,
					       d_interKeysSizePerTask,//store keysize of per task
					       d_interValsSizePerTask,//store valuesize of per task
					       d_interCountPerTask,//store keys count of per task
					       h_inputRecordCount, //total input record count
					       h_recordsPerTask,//default is 1
					       h_actualNumThreads);
	...
	//	 iii)	d_interCountPerTask
	//-----------------------------------------------
 	/*	prefix sum: sum all the elements before the current element.
 	 * 
 	 * 	d_psKeySizs: sum all the keysize before the current key.
 	 * 	d_psValSizes:sum all the valuesize before the current value.
 	 * 	d_psCounts:sum all the record count before the current record count.
 	 * 
 	 * the return is the sum of total elements in the array.
 	 * 
 	 * h_allKeySize	: sum of total keysize in the intermediate data.
 	 * h_allValSize 	: sum of total valuesize in the intermediate data.
 	 *   h_allCounts	: sum of totla record count in the intermediate data.
 	 * */
	...
	if (h_allCounts == 0)//if no intermediate data.
	{
		DoLog( "** No output.");
	...
	/*	allocate cuda memory according to the total size calculated above
	 * */
	...
	int sizeSmem = h_dimBlock.x * sizeof(int) * 5;//shared memory size in the block,each thread can have save 5 int variables
	Mapper<<<h_dimGrid, h_dimBlock, sizeSmem>>>(d_inputKeys,//input key
					  d_inputVals,//input value
					  d_inputOffsetSizes,//input offset
					  d_psKeySizes,//prefix sum of keysize
					  d_psValSizes,//prefix sum of valuesize
					  d_psCounts,//prefix sum of record count
					  d_keyValOffsets,//key and value size in each task
					  d_interKeys,//intermediate key
					  d_interVals,//intermediate value
					  d_interOffsetSizes,//intermediate key and value offset
					  d_curIndex,//current task record count
					  h_inputRecordCount, //input record count
					  h_recordsPerTask,//default 1 : each thread processes one key
					  h_actualNumThreads);
 	cudaThreadSynchronize(); 
 	
 	/*	after map we can get intermediate data : d_interKeys,d_interVals,d_interOffsetSizes
 	 * d_interKeys: intermediate key such as "abc", "bcd","adf a fad ". user can define it differently
 	 * d_interVals: intermediate value such as 1.
 	 * d_interOffsetSize: <keyoffset,keysize,valueoffset,valuesize>. 
 	 * */
	...
	//fprintf(stderr,"%d",g_spec->interRecordCount);
